...
class SortablePage(BasePage):
    ...
    @allure.step("Получить элементы сортировки")
    def get_sortable_items(self, elements):
        item_list = self.elements_are_visible(elements)
        return [item.text for item in item_list]

    @allure.step("Изменить порядок в списке")
    ...
    @allure.step("Изменить порядок в сетке")
...
class SelectablePage(BasePage):
    ...
    @allure.step("Кликнуть на выбираемый элемент ")
    def click_selectable_item(self, elements):
        item_list = self.elements_are_visible(elements)
        random.sample(item_list, k=1)[0].click()

    @allure.step("Выбрать элемент из списка")
    ...
    @allure.step("Выбрать элемент из сетки")
...
class ResizablePage(BasePage):
    ...
    @allure.step("Получить размеры из строки стиля")
    def get_px_from_width_height(self, value_of_size):
        width = value_of_size.split(';')[0].split(':')[1].replace(' ', '')
        height = value_of_size.split(';')[1].split(':')[1].replace(' ', '')
        return width, height

    @allure.step("Получить текущий размер элемента")
    def get_max_min_size(self, element):
        size = self.element_is_present(element)
        size_value = size.get_attribute('style')
        return size_value

    @allure.step("Изменить размер ограниченного блока")
    ...
    @allure.step("Изменить размер неограниченного блока")
...
class DroppablePage(BasePage):
    ...
    @allure.step("Перетаскивание с проверкой accept")
    ...
    @allure.step("Перетаскивание с revert")
...
class DraggablePage(BasePage):
    ...
    @allure.step("Получить позиции до и после перемещения")
    ...
    @allure.step("Ограниченное перемещение по Y")
